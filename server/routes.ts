...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Routes publiques pour la réservation client (sans authentification)
  ...
  app.get('/api/public-staff/:salonId', async (req, res) => {
    try {
      const { salonId } = req.params;
      const staff = await storage.getStaff(salonId);
      res.json(staff || []);
    } catch (error) {
  ...
  app.get('/api/business-info/:salonId', async (req, res) => {
    try {
      ...
      res.json({
        id: user.id,
        name: user.username || "Salon de Beauté",
  ...
  app.post('/api/public-booking', async (req, res) => {
    try {
      const { salonId, serviceId, date, time, clientInfo } = req.body;
      
      // Créer ou récupérer le client
      let client;
      try {
        const existingClients = await storage.getClients(salonId);
        client = existingClients.find(c => c.email === clientInfo.email);
        ...
        return res.status(400).json({ message: "Failed to process client information" });
      }
      ...
      const appointmentData = {
        ...
        startTime: time,
        status: "confirmed",
        notes: "Réservation via interface client"
      };

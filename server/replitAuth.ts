...
export function getSession() {
  ...
  const sessionStore = new pgStore({
    ...
    createTableIfMissing: true, // Permet de créer la table si elle n'existe pas
    ttl: sessionTtl,
    tableName: "sessions",
  });
  return session({
...
export async function setupAuth(app: Express) {
  ...
  app.get("/api/callback", (req, res, next) => {
    passport.authenticate(`replitauth:${req.hostname}`, {
      successReturnToOrRedirect: "/business-features",
      failureRedirect: "/api/login",
    })(req, res, next);
  });

  // Route logout Replit Auth désactivée - utilisation de /api/auth/logout unifié

...
export default function StaffManagementModern() {
  ...
  const [newStaff, setNewStaff] = useState<Partial<Professional> & { selectedServices?: string[]; jobTitle?: string }>({
    ...
    bio: "",
    specialties: [],
    selectedServices: [], // Nouveaux services sélectionnés
    jobTitle: "", // Intitulé exact du métier
    experience: "",
    rating: 5.0,
    reviewCount: 0,
                  ...
                  <div className="grid grid-cols-2 gap-4 py-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        placeholder="Ex: Coiffeur, Esthéticienne, Manucure..."
                      />
                    </div>
                    <div className="space-y-2">
                ...
                <Card key={member.id} className="bg-white rounded-2xl shadow-sm border-0 overflow-hidden hover:shadow-lg transition-all duration-200">
                  <CardContent className="p-6">
                    ...
                    <div className="flex flex-wrap gap-1 mb-4">
                      {member.selectedServices?.slice(0, 3).map((serviceId: string, index: number) => {
                        const service = salonData?.services?.find((s: any) => s.id === serviceId);
                        return service ? (
                      ...
                      {member.selectedServices && member.selectedServices.length > 3 && (
                        <UIBadge variant="secondary" className="text-xs bg-gray-100 text-gray-600">
                          +{member.selectedServices.length - 3}
                        </UIBadge>
                      )}
                    </div>

...
export default function ClientLogin() {
  ...
  // Fonction centralisée pour gérer les erreurs de connexion
  const handleLoginError = (email: string, isLogin: boolean) => {
    if (!isLogin) return;
    
    const currentAttempts = getStoredAttempts(email)?.attempts || 0;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (isLogin) {
          handleLoginError(formData.email, isLogin);
        } else {
          toast({

...
const safeParseJSON = (data: string | null, fallback: any = {}) => {
...
const getSalonInfo = (data: any, field: string, fallback: string) => {
  return data?.[field] || data?.salon?.[field] || fallback;
};
...
export default function BookingSuccess() {
  ...
  // Récupérer les données de la réservation de manière sécurisée
  const selectedProfessionalRaw = localStorage.getItem('selectedProfessional');
  const selectedDateTime = safeParseJSON(localStorage.getItem('selectedDateTime'));
  const selectedService = safeParseJSON(localStorage.getItem('selectedService'));
  const salonData = safeParseJSON(localStorage.getItem('salonData'));
  const bookingData = safeParseJSON(localStorage.getItem('bookingData'));
  
  // Récupérer les données depuis preBookingData (nouveau flux)
  const preBookingData = safeParseJSON(sessionStorage.getItem('preBookingData'));
  ...
  // Priorité à preBookingData (nouveau flux)
  if (preBookingData?.professionalName) {
    selectedProfessional = preBookingData.professionalName;
  } else if (selectedProfessionalRaw && selectedProfessionalRaw !== 'any') {
    try {
  ...
  const [salonName, setSalonName] = React.useState<string>(getSalonInfo(salonData, 'name', 'Salon'));
  const [salonAddress, setSalonAddress] = React.useState<string>(getSalonInfo(salonData, 'business_address', 'Adresse non renseignée'));
    ...
    if (!salonData?.name && publicSlug) {
      fetch(`/api/public/salon/${publicSlug}`)
        ...
        .then(payload => {
          ...
          if (s) {
            setSalonName(getSalonInfo(s, 'name', 'Salon'));
            setSalonAddress(getSalonInfo(s, 'business_address', 'Adresse non renseignée'));
  ...
  // Récupérer les données réelles du service avec validation
  // Priorité à bookingData (données sauvegardées après paiement)
  const serviceName = bookingData?.serviceName || preBookingData?.selectedServices?.map((s: any) => s.name).join(', ') || selectedService?.name || 'Service';
  const servicePrice = validatePrice(bookingData?.servicePrice || preBookingData?.totalPrice || selectedService?.price);
  const serviceDuration = bookingData?.totalDuration || preBookingData?.totalDuration || selectedService?.duration || 60;
  const totalServices = bookingData?.totalServices || preBookingData?.serviceCount || 1;
  
  // Calculer l'acompte (50% par défaut) avec validation
  const depositAmount = bookingData?.depositAmount || Math.round(servicePrice * 0.5 * 100) / 100;
  const remainingAmount = Math.max(0, servicePrice - depositAmount);
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-sm text-gray-500">
                {totalServices > 1 ? `${totalServices} prestations • ` : ''}{serviceDuration} min
              </p>
            </div>
